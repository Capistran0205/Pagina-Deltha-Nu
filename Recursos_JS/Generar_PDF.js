...
function setupImagePreview(inputId, previewId, imgId) {
  ...
  if (input && preview && img) {
    ...
    console.warn(`Alguno de los elementos para preview (input: ${inputId}, preview: ${previewId}, img: ${imgId}) no fue encontrado. La previsualización de imagen no funcionará.`);
  }
}
...
async function generatePDF(formData) {
  ...
  if (headerLogoElement && headerLogoElement.src) {
    try {
      ...
      if (logoData && logoData.data) {
        ...
        const logoY = 3;
        pdf.addImage(logoData.data, logoData.type, logoX, logoY, logoWidth, logoHeight);
        contentStartY = logoY + logoHeight + 2;
      } else {
        console.warn("No se pudo obtener la imagen del logo del header o convertirla a Base64.");
        contentStartY = 10;
      }
    } catch (error) {
  ...
  pdf.roundedRect(photoX - 2, photoY - 2, photoWidth + 4, photoHeight + 4, 2, 2, "F");
  ...
  contentStartY = Math.max(photoY + photoHeight, additionalImageY + additionalImageHeight) + 5;
  ...
  for (const [index, persona] of personasData.entries()) {
    ...
    pdf.roundedRect(fotoX - 0.5, fotoY - 0.5, fotoSize + 1, fotoSize + 1, 0.5, 0.5, "F");
...
document.getElementById("formulario").addEventListener("submit", async function (e) {
  ...
  for (const campo of campos) {
    ...
    if (valor.length > 0 && !validarNombre(valor)) {
      alert(`El campo "${campo.label}" solo puede contener letras y espacios.`);
